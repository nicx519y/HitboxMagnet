...
//  MX_QUADSPI_Init();
  ...
  int index = 0;
	...
	// WS2812B_SetLEDColor(255, 0, 0, index);

  WS2812B_SetLEDColor(0, 0, 255, 5);
  WS2812B_SetLEDColor(0, 0, 255, 8);
	// int err;

	// QSPI_W25Qxx_ChipErase();
	// f_mkfs("0:", 0, formatBuffer, sizeof(formatBuffer));
  FRESULT res = f_mount(&fs,"0:",1);
  if(!res) {
		printf("mount success 1!\n");
	  } else {
	  f_mkfs("0:", 0, formatBuffer, sizeof(formatBuffer));
	  res = f_mount(&fs,"0:",1);
	  if(!res) {
		printf("mount success 2!\n");
	  } else {
		printf("mount failure! err=%d.\n", res);
	  }
	}
